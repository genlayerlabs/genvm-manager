...
from tools.versions import manager_version
...
class ProvisionBranches(ci_lib.Tool):
	...
	def add_to(self, parser: argparse.ArgumentParser) -> None:
		# Only the manager repo is relevant here.
	...
	def handler(self, args: argparse.Namespace) -> int:
		...
		if existing:
			print(f'Standing PR {dev_branch} -> {version_branch} already open (#{existing})')
			return 0
		...
		gh(
			...
			dev_branch,
			'--title',
			# The PR-title check validates this before the E2E App squash-merges it.
