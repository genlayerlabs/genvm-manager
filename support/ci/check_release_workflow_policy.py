...
def main() -> None:
	...
	for workflow in workflows:
		for uses in USES_PATTERN.findall(workflow.read_text()):
			...
			if not SHA_REF_PATTERN.fullmatch(ref):
				fail(f'{workflow.relative_to(ROOT)} uses mutable external action ref: {uses}')

	release = ENTRY.read_text()
	for forbidden in ('git push origin "$TAG"', 'git tag "$TAG"'):
		if forbidden in release:
			fail(
				f'release publication creates the tag before the draft is complete: {forbidden}'
			)
	...
	for fragment in required:
		if fragment not in release:
			fail(
				f'release workflow is missing required immutable-publication control: {fragment}'
			)
