...
class IntegrationSingleStep(genvm_tool.tests.exec.step.Python):
	...
	async def _run_single_step(
		...
		if is_leader:
			...
			# `leader_nondet` fills a well-formed envelope; this one replaces the
			# envelope itself, which is the only way to reach the decode failure
			raw_envelope = single_conf.get('leader_public_data_raw', None)
			if raw_envelope is not None:
				leader_public_data = bytes(raw_envelope)
			elif leader_nondet_fixture is None:
		...
		# compares. It folds kind (with fatality coalesced away), data, backtrace,
		# storage deltas, sub-VM hashes, wasm store hashes, both fee counters and
		# emissions -- so a mode that emits different messages fails here, not
		# only in the main-mode `messages` semantics component.
