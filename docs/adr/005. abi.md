...
Contracts need a standardized ABI (Application Binary Interface). Everything is encoded as [calldata](./004.%20calldata.md), but this calldata must have a defined structure. Similar to how POSIX defines `_start` as a program's entry point, GenVM needs standardized contract interfaces.
...
The method name is stored under the empty key `""` so that it sorts before
every other key in the (sorted) calldata encoding.
...
### Schema Retrieval
Special method call with `"": "#get-schema"` returning JSON-schema-like structure.
