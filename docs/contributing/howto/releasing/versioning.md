...
A platform release asset bundles every active line, and nothing else ships an
executor on its own: the executor repo publishes no releases, so an install root
gets its lines from the manager asset alone
