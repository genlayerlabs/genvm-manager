...
2. Set dev-mode back to `false`, and the changed runner's hash, plus its
   dependents', to `null`
3. Refresh the hashes with `runners/support/versions/hash-updater.py`
4. Rebuild to confirm no mismatch remains

Step 3 in detail, and why a hash is never restored by hand:
