...
1. `crates/modules-interfaces/codegen/data/host-fns.json` — add the method id,
   then `ninja -C build codegen` ([genvm-tool.md](../genvm-tool.md)). That
   regenerates `crates/modules-interfaces/src/host_fns.rs` and
   `tests/runner/origin/host_fns.py`
